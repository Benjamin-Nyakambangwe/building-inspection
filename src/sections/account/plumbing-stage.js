...
const NOTIFICATIONS = [
  {
    ...
    items: [
      {
        id: 'leak_free_and_correctly_sized_water_supply_lines',
        label: 'Water supply lines are leak-free and sized correctly.',
      },
      {
        id: 'proper_slope_and_venting_in_drainage_systems',
        label: 'Drainage systems have proper slope and venting. ',
      },
      {
        id: 'accurate_installation_of_plumbing_fixtures',
        label: 'Plumbing fixtures are correctly installed',
      },
      {
        id: 'use_of_code_compliant_pipe_materials',
        label: 'Pipe materials meet code requirements.',
      },
      {
        id: 'presence_of_required_p_traps_and_cleanouts',
        label: 'P-traps and cleanouts are in place.',
      },
      {
        id: 'compliance_with_local_plumbing_codes',
        label: 'Plumbing work complies with local codes',
      },
      {
        id: 'successful_pressure_testing',
        label: 'Pressure testing is performed and passes. ',
      },
      {
        id: 'installation_of_backflow_prevention_devices_where_required',
        label: 'Backflow prevention devices are installed where necessary.',
      },
    ],
  },
  //   {
...
export default function AccountNotifications({ data, client_id }) {
  ...
  const [notes, setNotes] = useState(data.plumbing_inspection[0].notes);
  const [stageCompleted, setStageCompleted] = useState(data.plumbing_inspection[0].is_completed);
  ...
  const handleStageChange = (event) => {
    ...
    if (newData.plumbing_inspection && newData.plumbing_inspection[0]) {
      newData.plumbing_inspection[0] = {
        ...newData.plumbing_inspection[0],
      ...
      console.log('ONCHANHE', currentClientData.plumbing_inspection[0]);
  ...
  console.log('plumbing data', data.plumbing_inspection[0]);
  ...
  const filteredObject = filterTrueValues(data.plumbing_inspection[0]);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    const res = await fetch('http://127.0.0.1:8000/client/update-plumbing/', requestOptions);
  ...
  return currentClientData.current_stage > 3 ? (
    <FormProvider methods={methods} onSubmit={onSubmit}>
              ...
              >
                ...
                <Box sx={{ p: 1, borderRadius: 2, bgcolor: 'background.neutral' }}>
                  ...
                  <FormControlLabel
                    ...
                    control={
                      <Switch
                        checked={currentClientData?.plumbing_inspection?.[0]?.is_completed}
                        ...
                        <FormControlLabel
                          ...
                          control={
                            <Switch
                              ...
                              checked={currentClientData?.plumbing_inspection?.[0]?.[item.id]}
      ...
      <Stack component={Card} spacing={3} sx={{ p: 3, mt: 5 }}>
        <Grid container spacing={2}>
          <Grid xs={12} md={6}>
            {data?.plumbing_inspection[0].image1 ? (
              <Image src={data?.plumbing_inspection[0].image1} />
            ) : (
              <Typography>Please Upload First Image</Typography>
            )}
          </Grid>
          <Grid xs={12} md={6}>
            {data?.plumbing_inspection[0].image2 ? (
              <Image src={data?.plumbing_inspection[0].image2} />
            ) : (

...
export default function CalendarView() {
  ...
  const { events, eventsLoading } = useMockGetEvents();
...
const useMockGetEvents = () => {
  ...
  useEffect(() => {
    const getDBEvents = async () => {
      ...
      const res = await fetch('http://127.0.0.1:8000/client/get-events', requestOptions);
    ...
    //     end: new Date().setDate(new Date().getDate() + 1), // Tomorrow
    //     color: '#0000FF',
    //   },

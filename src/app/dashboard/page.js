...
async function getDataFromMultipleEndpoints() {
  try {
    const endpoint1 = 'http://127.0.0.1:8000/client/top-rated';
    const endpoint2 = 'http://127.0.0.1:8000/client/profiles-month';
    const endpoint3 = 'http://127.0.0.1:8000/client/count-profiles';
    ...
    const responses = await Promise.all([
      ...
      fetch(endpoint1, { cache: 'no-store' }),
      fetch(endpoint2, { cache: 'no-store' }),
      fetch(endpoint3, { cache: 'no-store' }),
